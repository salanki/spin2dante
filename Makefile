...
test-volume: build inferno2pipe
	cd test && docker compose -f docker-compose.volume.yml down --remove-orphans 2>/dev/null; \
	docker compose -f docker-compose.volume.yml up --build --abort-on-container-exit validator; \
	result=$$?; \
	docker compose -f docker-compose.volume.yml down --remove-orphans; \
	exit $$result
